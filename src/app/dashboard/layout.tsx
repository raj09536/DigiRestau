...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    const [isMandatoryUpgrade, setIsMandatoryUpgrade] = useState(false);
    ...
    // Fetch active orders count for badge
    useEffect(() => {
    ...
    const navItems = [
        ...
        { href: '/dashboard/templates', label: 'Menu Designs', icon: Palette },
        { href: '/dashboard/tables', label: 'Tables', icon: Grid3X3, minPlan: 'pro' },
        { href: '/dashboard/feedback', label: 'Feedback', icon: MessageSquare, minPlan: 'pro' },
        ...
        if (item.minPlan) {
            const minTierIdx = tiers.indexOf(item.minPlan);
            if (currentTierIdx < minTierIdx) return false;
        }
                ...
                <div className="flex flex-col h-full">
                                ...
                                >
                                    ...
                                    {item.badge && !active && (
                                        <span className="ml-auto flex items-center gap-1.5">
                                            <span className="min-w-[20px] h-5 px-1.5 rounded-full bg-saffron text-white text-[10px] font-black flex items-center justify-center">
                    ...
                    <div className="p-6 border-t border-white/5 space-y-4">
                        {restaurant?.plan_tier !== 'max' && (
                            ...
                            >
                                ...
                                <div className="upgrade-box-text flex-1">
                                    <strong>{restaurant?.plan_tier === 'pro' ? 'Upgrade to Max' : 'Upgrade to Pro'}</strong>
                                    <span>{restaurant?.plan_tier === 'pro' ? 'Unlimited Everything' : 'Enable Orders & Tables'}</span>
                                </div>
                        ...
                        <div className="px-4 py-3 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center gap-2 group cursor-default">
                            <Star className={`w-4 h-4 ${restaurant?.is_premium ? 'text-gold' : 'text-text-muted opacity-30'}`} />
                            <span className="text-[10px] font-black uppercase tracking-widest text-text-muted">
                                {restaurant?.plan_tier ? `${restaurant.plan_tier} Plan` : 'No Active Plan'}
                            </span>
                        </div>
