...
export default function TeamPage() {
    ...
    const handleAddStaff = async (e: React.FormEvent) => {
        ...
        try {
            console.log('Sending staff data:', JSON.stringify(data, null, 2));
            ...
            if (error) {
                console.error('Staff Action Error Detailed:', JSON.stringify(error, null, 2));
                ...
                setStaffForm({
                    ...
                    salary_type: 'fixed', salary_amount: 0, shift_type: 'fullday'
                });
                fetchData();
            }
        } catch (err: any) {
                        ...
                        <div className="grid grid-cols-3 gap-4 mb-8">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <select className="w-full bg-white/5 border border-white/10 rounded-2xl p-4" value={staffForm.shift_type} onChange={e => setStaffForm({...staffForm, shift_type: e.target.value})}>
                                    <option value="fullday">Full Day (9-9)</option>
                                    <option value="morning">Morning (9-3)</option>
                                    <option value="evening">Evening (3-9)</option>
                                </select>
                            </div>
                        </div>

                        <button type="submit" className="w-full py-5 bg-saffron text-white rounded-2xl font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-saffron/30">
