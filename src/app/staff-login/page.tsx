...
export default function StaffLoginPage() {
    ...
    const handleStaffLogin = async (e: React.FormEvent) => {
        ...
        try {
            // Resolve restaurant ID - support both short ID (8 chars shown on dashboard) and full UUID
            let resolvedRestaurantId = restaurantId.trim();
            
            // If it looks like a short ID (not a full UUID), find the matching restaurant
            if (resolvedRestaurantId.length < 36) {
                const { data: restaurants, error: restError } = await supabase
                    .from('restaurants')
                    .select('id, name, slug');

                const match = restaurants?.find(r => 
                    r.id.toUpperCase().startsWith(resolvedRestaurantId.toUpperCase())
                );

                if (restError || !match) {
                    setError('Invalid Restaurant ID. Please check and try again.');
                    setLoading(false);
                    return;
                }
                resolvedRestaurantId = match.id;
            }

            // Find staff member with resolved restaurant ID
        ...
        <div className="staff-login-container selection:bg-saffron selection:text-white">
            <div className="staff-login-card animate-fade-in">
                <div className="staff-login-header">
                    ...
                    <p className="staff-login-subtitle">Login to your restaurant</p>
                </div>
